...
void CacheSim::init(_u64 a_cache_size, _u64 a_cache_line_size, _u64 a_mapping_ways, cache_swap_style a_swap_style) {
   //如果输入配置不符合要求
    ...
    // SHIP init, takes 2^21 * 3 / 8 = 768 KB, extra information in cacheLine takes 2^16 * 5 / 8 = 40 KB
    // 3 bit for a signature in counter, 3 bit for RRIP + 1 bit for outcome + 2 bit from CPU/GPU in cacheLine
    SHCT = new char[1 << 21];
    memset(SHCT, 0, 1 << 21);
...
_u64 CacheSim::calc_addr(_u64 set_base, _u64 tag){
    _u64 prefix = set_base / cache_mapping_ways; // set_base = cache row index * mapping_ways
    _u64 ans = tag << (cache_set_shifts);
    ans &= prefix;
    ans <<= cache_line_shifts;
    return ans;
}

/**Normal Cache命中，更新Cache状态*/
void CacheSim::cache_hit(_u64 set_base, _u64 index, int a_swap_style, _u64 addr, int source) {
        ...
        switch (a_swap_style) {
                ...
                case CACHE_SWAP_SHIP:
                    ...
                    // }
                    // else{ // reference from gpu
                    //     caches[set_base + index].gpu = 1;
...
void CacheSim::cache_insert(_u64 set_base, _u64 index, int a_swap_style, _u64 addr, int source) {
        double rand_num; 
        switch (a_swap_style) {
                case CACHE_SWAP_LRU:
                    for (_u64 j = 0; j < cache_mapping_ways; ++j) {
                ...
                case CACHE_SWAP_SHIP:
                    ...
                    // }
                    // else{
                    //     caches[set_base + index].cpu = 0;
                    //     caches[set_base + index].gpu = 1;
                    // }
                    // if(SHCT[SHCT_index(addr, source)])
                    //     caches[set_base + index].RRPV = SRRIP_2_M_2;
                    // else
                    //     caches[set_base + index].RRPV = SRRIP_2_M_1;
                    if(SHCT[SHCT_index(addr)])
                        caches[set_base + index].RRPV = SRRIP_2_M_2;
                    else
                        caches[set_base + index].RRPV = SRRIP_2_M_1;
                    break;
...
int CacheSim::cache_find_victim(_u64 set_base , int a_swap_style, int hit_index) {
   ...
   if (free_index >= 0) {
        if(a_swap_style == CACHE_SWAP_SHIP){
            ...
            //         SHCT[SHCT_index(addr, cpu_mask)]--;
            //     if(caches[set_base + free_index].gpu && SHCT[SHCT_index(addr, gpu_mask)])
            //         SHCT[SHCT_index(addr, gpu_mask)]--;
            // }
            if(!caches[set_base + free_index].outcome && SHCT[SHCT_index(addr)])
                SHCT[SHCT_index(addr)]--;
        }
    ...
    */
    ...
    if (oper_style == OPERATION_WRITE) {
        if (hit_index >= 0) {
            ...
            //更新替换相关状态位
            cache_hit(set_base, hit_index, temp_swap_style, addr, source);
        } else {
            cache_w_miss_count++;
            cache_miss_count++;
            ...
            if (write_allocation) {
                ...
                if (write_through) {
                 ...
                 //更新替换相关的状态位
                 cache_insert(set_base, free_index, temp_swap_style, addr, source);
...
void CacheSim::load_trace(const char *filename) {
    ...
    while (fgets(buf, sizeof(buf), fin)) {
        ...
        // is from cpu or gpu
        mid = (mid == 0x78 || mid == 0x79) ? cpu_mask : gpu_mask;
        //访问cache
        do_cache_op(addr, style, mid);
