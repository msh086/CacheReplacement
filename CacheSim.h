...
public:
    ...
    _u32 RRPV;
    _u32 FRU;
    _u8 outcome; // whether this cache line has been re-referenced
    _u8 cpu; // from cpu
    _u8 gpu; // from gpu
};
...
public:
    ...
    /**命中，更新Cache状态*/
    void cache_hit(_u64 set_base, _u64 index, int a_swap_style, _u64 addr, int source);
    /**缺失，插入新行后更新Cache状态*/
    void cache_insert(_u64 set_base, _u64 index, int a_swap_style, _u64 addr, int source);
    void dump_cache_set_info(_u64 set_base);

    /**对一个指令进行分析*/
    void do_cache_op(_u64 addr, char oper_style, int source);
